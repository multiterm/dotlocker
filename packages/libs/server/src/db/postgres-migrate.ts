...
export async function migratePostgres(sql: postgres.Sql): Promise<void> {
  await sql.begin(async (tx) => {
    ...
    await tx`CREATE TABLE IF NOT EXISTS public_files (id text PRIMARY KEY, path text NOT NULL UNIQUE REFERENCES file_records(path) ON DELETE CASCADE, org text NOT NULL REFERENCES orgs(name) ON DELETE CASCADE, created_at bigint NOT NULL, created_by text REFERENCES users(email))`;
    await tx`CREATE INDEX IF NOT EXISTS public_files_org_idx ON public_files(org, created_at)`;
