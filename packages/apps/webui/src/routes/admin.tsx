        ...
        <Dialog title="Provision Keyname user" onClose={() => setModal(null)}>
          ...
          >
            ...
            <p className="muted text-sm">
              The email must match a verified Keyname identity. No dot.locker password is created.
            </p>
...
function TokensPage() {
  ...
  const [sessions, setSessions] = useState<ApiKeyRecord[]>([]);
  ...
  return (
    <>
      <section className="card p-0">
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-[var(--pl-line)] px-5 py-4">
          <div>
            ...
            <p className="mb-0 mt-1 text-xs text-[var(--pl-muted)]">
              {loading ? "Loading API keys…" : `${keys.length} keys in ${org}`}
            </p>
          </div>
        ...
        <div className="border-b border-[var(--pl-line)] bg-[var(--pl-surface)] px-5 py-3 text-xs text-[var(--pl-muted)]">
          Local dot.locker configuration uses an API key as its only authentication credential. Each
          key grants explicit path and access rights.
        </div>
        {error && !modal && (
              ...
              {keys.length ? (
                keys.map((key) => {
                  ...
                  return (
                    <tr key={key.id}>
                      <td>
                        <b className="text-[var(--pl-text)]">{key.label || "Unnamed key"}</b>
                      </td>
                      <td>
                        <code>{key.id}</code>
                      </td>
                      <td>
                        {key.scopes.map((scope) => (
                          <code key={`${scope.path}:${scope.access}`} className="block">
                            {scope.path}
                          </code>
                        ))}
                      </td>
                      <td>
                        {key.scopes.map((scope) => (
                          <span key={`${scope.path}:${scope.access}`} className="pill capitalize">
                            {scope.access}
                          </span>
                        ))}
                      </td>
                      <td>
                        <span
                          className={`pill ${status === "Active" ? "text-[var(--pl-success)]" : "text-[var(--pl-subtle)]"}`}
                        >
                          {status}
                        </span>
                      </td>
      ...
      <section className="card p-0">
        <header className="border-b border-[var(--pl-line)] px-5 py-4">
          <h3 className="mb-0">Active sessions</h3>
          <p className="mb-0 mt-1 text-xs text-[var(--pl-muted)]">
            Keyname browser sessions visible to you in {org}. Revoke a session to require sign-in
            again.
          </p>
        </header>
        <div className="overflow-x-auto">
          <table className="mt-0 min-w-[760px] rounded-none border-0">
            <thead>
              <tr>
                <th>Identity</th>
                <th>Session</th>
            ...
            <tbody>
              {sessions.map((session) => (
                <tr key={session.id}>
                  <td>{session.userEmail ?? "Unknown"}</td>
                  <td>
                  ...
                  <td>
                    <span className="pill">{statusFor(session)}</span>
                  </td>
                  <td>
                    <button
                      className="secondary inline"
                      disabled={statusFor(session) !== "Active"}
                      onClick={() => {
                        setSelected(session);
                        setError("");
                        setModal("revoke");
              ...
              {!sessions.length && (
                <tr>
                  <td colSpan={6} className="py-10 text-center text-[var(--pl-muted)]">
        ...
        <Dialog title="Create API key" onClose={() => setModal(null)}>
          <form onSubmit={(event) => void createKey(event)}>
            <p className="mt-0 text-sm text-[var(--pl-muted)]">
              Create an API key and grant only the access rights required by this local
              configuration.
            </p>
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={allRepos}
                onChange={(event) => setAllRepos(event.target.checked)}
                className="h-4 w-4"
              />{" "}
              All repositories and all runtimes
            </label>
            <div className="row">
              <div>
                <label htmlFor="key-repo">Repository</label>
                <input
                  id="key-repo"
                  required={!allRepos}
                  disabled={allRepos}
              ...
              <div>
                <label htmlFor="key-runtime">
                  Runtime <span className="font-normal text-[var(--pl-subtle)]">(optional)</span>
                </label>
                <input
                  id="key-runtime"
                  disabled={allRepos}
            ...
            <div className="row">
              <div>
                <label htmlFor="key-access">Access rights</label>
                <select
                  id="key-access"
                  value={access}
            ...
            <div className="mt-4 rounded-[var(--pl-radius-xs)] border border-[var(--pl-line)] bg-[var(--pl-surface)] p-3 text-xs text-[var(--pl-muted)]">
              Grant:{" "}
              <code>
                {allRepos
                  ? `${org}/**`
                  : [org, repo || "<repository>", runtime || undefined, "**"]
                      .filter(Boolean)
                      .join("/")}
              </code>{" "}
              ·{" "}
              <b className="capitalize text-[var(--pl-text)]">
      ...
      {modal === "created" && (
        <Dialog title="API key created" onClose={() => setModal(null)}>
          <div className="rounded-[var(--pl-radius-xs)] border border-[var(--pl-success)]/40 bg-[var(--pl-success)]/5 p-3 text-sm text-[var(--pl-muted)]">
          ...
          <textarea
            ...
            readOnly
            rows={3}
            className="w-full resize-none rounded-[var(--pl-radius-xs)] border border-[var(--pl-line)] bg-[var(--pl-bg)] p-3 font-mono text-xs text-[var(--pl-text)] outline-none"
            value={plaintext}
          />
          <p className="text-xs text-[var(--pl-muted)]">
            Use it locally as <code>DOTLOCKER_TOKEN</code>. No username or password is required.
          </p>
          <div className="actions justify-end border-t border-[var(--pl-line)] pt-4">
            <button
              className="secondary"
      ...
      {modal === "revoke" && selected && (
        ...
        >
          <p className="mt-0 text-sm text-[var(--pl-muted)]">
            Revoke{" "}
            <b className="text-[var(--pl-text)]">
              {selected.label || selected.userEmail || selected.id}
            </b>
            ?{" "}
            {sessions.some((session) => session.id === selected.id)
              ? "That browser will need to sign in again."
              : "Local configurations using this key will immediately lose access."}
          </p>
          {error && <p className="err mt-3 text-xs">{error}</p>}
          <div className="actions justify-end border-t border-[var(--pl-line)] pt-4">
            ...
            <button className="danger" disabled={submitting} onClick={() => void revokeKey()}>
              {submitting ? "Revoking…" : "Revoke API key"}
            </button>
          </div>
        </Dialog>
      )}
  ...
  return (
    <div className="card">
      ...
      <div className="row">
        ...
        <div>
          <button onClick={() => void api.audit(org, limit).then(setOut).catch(setOut)}>
            Load audit
          </button>
        </div>
      </div>
  ...
  return (
    <div className="grid cols-2">
      <div className="card">
        <h3>Connection</h3>
        <p className="muted">
          dot.locker uses the authenticated same-origin connection. Organizations are limited to
          your current grants.
        </p>
        ...
        <select id="settings-org" value={org} onChange={(event) => setOrg(event.target.value)}>
          {(s.me?.orgs.length ? s.me.orgs : [s.org]).map((name) => (
      ...
      <div className="card">
        ...
        <p className="muted">
          Signed in as{" "}
          <b className="text-[var(--pl-text)]">
            {s.me?.identityEmail ?? s.me?.token.userEmail ?? "Loading…"}
          </b>
          . Signing out also closes the active Keyname session.
        </p>
        <button
          className="danger"
          onClick={async () => {
